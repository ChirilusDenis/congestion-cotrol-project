...
#define VEGAS_ALPHA 3
#define VEGAS_BETA 2
#define VEGAS_WMAX 10000
...
void CCSrc::processAck(const CCAck& ack) {
    ...
    // TCP Compound: Adjust congestion window using both Reno and Vegas
    ...
    _ssthresh = max(static_cast<int>(_cwnd / 2), 2 * _mss);

    // Vegas part
    if (_cwnd < VEGAS_WMAX * _mss) {
        double increase = VEGAS_ALPHA * _mss * _mss / _cwnd;
        _cwnd += std::min(static_cast<double>(_mss), increase);
    }

    _cwnd = std::max(MIN_CWND * _mss, std::min(static_cast<int>(_cwnd), MAX_CWND * _mss));
...
void CCSrc::processNack(const CCNack& nack) {
    ...
    // Adjust congestion window based on NACKs if necessary
    if (nack.ackno() >= _next_decision) {
        _ssthresh = max(static_cast<int>(_cwnd / 2), 2 * _mss); // Cast to int to match types
        _cwnd = _cwnd * .5; // Multiplicative decrease
